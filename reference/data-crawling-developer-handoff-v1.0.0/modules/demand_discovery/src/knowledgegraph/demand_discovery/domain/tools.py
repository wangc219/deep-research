...
from knowledgegraph.demand_discovery.domain.source_health import SourceHealthSnapshot
...
def build_all_tools(
    ...
    *,
    ...
    acquisition_adapters: list[SourceAcquisitionAdapter] | None = None,
    ...
    resolved_acquisition_adapters = (
        list(acquisition_adapters)
        if acquisition_adapters is not None
        else [
            MlplaJournalAdapter(fetch_http_transport),
            EntryUrlProbeAdapter(fetch_http_transport),
        ]
    )
    acquired_document_store = AcquiredDocumentStore()
    return [
        create_acquire_whitelist_documents_tool(
            source_registry,
            artifacts,
            adapters=resolved_acquisition_adapters,
            document_store=acquired_document_store,
            source_health_snapshot=source_health_snapshot,
        ),
        create_read_acquired_document_tool(acquired_document_store, artifacts),
        create_search_sources_tool(
            source_registry,
            gate,
        ...
        _object_schema(
            ...
            properties={
                ...
                | {
                    ...
                    "x-enumAliases": dict(_CANDIDATE_STATUS_ALIASES),
                },

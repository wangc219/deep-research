...
S3_S4_WEAPON_NAMING_TYPES: tuple[dict[str, str], ...] = (
    {"code": "A", "label": "构型意象型", "focus": "突出独特物理形态、结构构型、外观或仿生构型"},
    {"code": "B", "label": "原理突破型", "focus": "突出新物理、新材料、新机制或非传统技术原理"},
    {"code": "C", "label": "装备专名型", "focus": "采用未来列装装备的正式专名或代号，并保留具体武器本体"},
    {"code": "D", "label": "使命任务型", "focus": "突出装备专门解决的关键军事任务或战场矛盾"},
    {"code": "E", "label": "能力意象型", "focus": "用凝练意象表达装备最核心、最具威慑力的作战能力"},
    {"code": "F", "label": "作战机制型", "focus": "突出自组织、协同、分布式或涌现等作战方式变化"},
    {"code": "G", "label": "材料介质型", "focus": "突出构成核心能力的特殊材料、物质形态或作用介质"},
    {"code": "H", "label": "环境融合型", "focus": "突出装备与自然环境、战场环境或跨介质边界的融合"},
    {"code": "I", "label": "动作行为型", "focus": "以独特运动、行动或接敌行为作为名称核心"},
    {"code": "J", "label": "时空概念型", "focus": "突出对响应时间、作用距离、空间范围或存在方式的改变"},
    {"code": "K", "label": "体系化武器型", "focus": "突出单件武器作为可部署、可协同、可消耗的体系作战单元"},
    {"code": "L", "label": "反传统隐喻型", "focus": "借自然、生物、工业过程或抽象概念形成强烈认知隐喻"},
    {"code": "M", "label": "数量规模型", "focus": "突出数量、密度、规模、可消耗性本身形成的战斗力"},
    {"code": "N", "label": "经济学颠覆型", "focus": "突出成本交换、消耗逻辑或防御经济学的颠覆"},
    {"code": "O", "label": "演化代际型", "focus": "突出相对传统装备的代际跃迁、范式变化或能力越界"},
)
...
def _s3_s4_naming_assignment(seed: str, *, count: int = 2) -> dict[str, Any]:
    ...
    return {
        ...
        "application_rule": (
            "候选按输出顺序采用相同candidate_position的类型；未输出的位置忽略。"
            "不得交换、合并或在名称中写类型标记。"
        ),
    }
...
def _targeted_expert_feedback(
    ...
    for item in feedback:
        ...
        targets = {
            ...
            for value in (raw_targets if isinstance(raw_targets, (list, tuple)) else [raw_targets])
            if str(value).strip()
        }
...
def _creative_s3_candidate_instruction() -> str:
    ...
    return (
        "你是新质武器概念首席创造者，从Query的核心战场矛盾自由创造；先在内部推演战场矛盾→接敌关系→构型→作用对象→直接战果，不按装备目录、技术词表、固定维度或名称模板凑数。"
        "候选须是可单独指认、研发、部署和验收的物理武器本体，能直接打击、毁伤、压制、拦截或拒止；不能把火控节点、目标标记器、航迹灯、通信中继、算法或保障单独充当主体，但可内生到发射、携带或释放效应载荷的弹药。允许前沿构型但不得违反已知物理；核心效应须有合理作用载体并能落到武器本体。像军事装备专家一样先判断任务对象、作用载体、接敌几何、作战时序或对手代价，再闭合本体、战果和敌方适应；不要从名称反推机理。静默检查科学可实现性，本阶段不写工程瓶颈、成熟度、参数或验证边界。面向未来战争，新质方案须改写交战关系并形成直接战果。"
        "名称须识别武器本体，可从独特物理形态、结构构型、材料/介质、新物理原理、装备独特运动方式、反传统隐喻、数量/密度/规模自然生长；一般性的任务能力、功能效果和动作流程应进入concise_winning_summary。‘背景剥离诱显巡飞弹’、‘断链自证巡飞弹’属于功能/动作短语直接粘装备尾词的反例，‘裂隙蜂群巡飞弹’若无独有构型或作战关系也是意象换皮。"
        "每个候选只输出name和concise_winning_summary；名称和摘要共同说清具体作用、作用对象、核心机理和直接战果。每轮输入中的random_naming_style_assignment会从‘新质武器装备命名类型体系’A—O随机抽取命名方式，覆盖形态物质、技术原理、任务能力、战争时空与体系经济逻辑、专名隐喻等表达；按候选顺序一一采用，两个候选不得使用同一类型。随机类型只决定名称的主导表达方式，不得改变已经推演成立的武器本体和制胜机理，也不得把类型字母或类型名写进标题；不要为覆盖类型而组合多个不相干特征。名称采用‘创新核心＋具体武器本体’的真实装备语感，创新核心须针对Query自行发现，不能套用示例或词库；不要默认两字意象加弹/雷/器/系统。名称主体尽量控制为8—12个汉字（中文引号和标点不计），优先简洁、具体、可识别；复杂任务、机理和战果放入concise_winning_summary。"
        "不要使用‘环境/威胁条件＋自证/自校正/复获/续接＋装备后缀’的机械句式；‘玄磁’（“玄磁”超材料隐身巡弋器）、等离子流控飞行器仅为可解释示例，不得机械复用；专名/代号型不得成为明显多数，确有代号时只引专名/代号部分，名称没有专名或代号时，不得给整个名称加引号。重复底名换皮不算多样。若只是普通改型、支援节点或无法闭合的幻想，返回空hypotheses并说明停止原因。只输出严格JSON，不要输出其他字段、备选名或推理过程。"
    )
...
def _s3_s4_quality_first_instruction() -> str:
    ...
    return (
        "不要扩写清单，也不要把开放镜头当题目。先在内部比较至少三种不同的装备架构和对手反应；"
        "本次调用可提交1—2个候选：当Query确实包含两条彼此独立的接敌关系、作用原理或战场时空时，优先提交两个，"
        "否则只提交最强的一个。两个候选必须是不同武器本体和不同制胜关系，不能是同一平台换载荷、换代号或换任务前缀。"
        "为形成约20项竞争池，后续Codex迭代会继续产生新的独立候选；这不是机械配额，宁缺毋滥，宁可返回空hypotheses，也不要为了凑数输出普通改型。"
        "若本轮没有达到新质断点，返回空hypotheses，控制器会再次调用S3/S4让你换一条路线重新创造。"
        "命名须依次采用输入中随机分配的A—O命名类型；若返回两个候选，两种命名类型必须不同。"
        "名称主体尽量为8—12个汉字（中文引号和标点不计），但不要为压缩长度牺牲具体武器本体。"
        "不要默认两字意象加弹/雷/器/系统，也不要把字段拼成标题。六项语义只作为内部语义脊柱理解，不要求逐项填表。"
    )
...
def _s3_s4_name_authoring_issues(name: Any) -> list[str]:
    ...
    return ["placeholder_or_nonfinal_marker"] if any(marker in normalized for marker in markers) else []
...
def _minimal_s6_card_handoff(brief: Mapping[str, Any]) -> dict[str, Any]:
    ...
    fields = (
        ...
        "failure_boundary", "concise_winning_summary", "innovation_basis",
        "frontier_principle", "technology_discontinuity", "core_disruptive_difference",
...
def _dynamic_s6_card_input(
    ...
    *,
    ...
    logic summary.  The frozen card_binding_id travels inside candidate_weapon
    as a routing key only: runtime uses it to create a separate ephemeral
    Codex CLI provider for this card without expanding the model's business
    context. Indicators, classifications, evidence, validation and portfolio
    metadata stay outside the model call and are never allowed to anchor the
    portrait.
    """
...
def _dynamic_s6_input_fingerprint(payload: Mapping[str, Any]) -> str:
    """Identify the exact semantic input that owns a reusable S6 card."""
...
def _minimal_portfolio_candidate_handoff(
    ...
    *,
    ...
    if not rich:
        return result
    equipment_form = str(
        _winning_primary_equipment_form(item) or item.title
    ).strip()
    result.update(
        {
    ...
    semantic_parts = [
        ...
        "；".join(str(value).strip() for value in item.direct_military_effects[:2] if str(value).strip()),
    ]
...
def _dynamic_portfolio_innovation_priority(
    ...
    authored = sum(
        bool(str(getattr(item, key, "") or "").strip())
...
def _s5_dimension_scores(value: Mapping[str, Any]) -> tuple[dict[str, float], float]:
    ...
    aliases = {
        "innovation": ("innovation", "创新性"),
        "demand": ("demand", "需求性"),
        "feasibility": ("feasibility", "可行性", "scientific_feasibility", "科学可行性"),
        "effectiveness": ("effectiveness", "效能性"),
        "development": ("development", "发展性"),
    }
    ...
    for dimension, keys in aliases.items():
        ...
        if candidate in (None, ""):
            candidate = value.get("innovation_priority") if dimension == "innovation" else 0.5
        try:
    ...
    weighted = sum(
        ...
        for dimension, weight in S5_DIMENSION_WEIGHTS.items()
    )
...
def _s5_disruption_tier_rank(
    ...
    try:
        priority = float(innovation_priority) if innovation_priority is not None else 0.0
    except (TypeError, ValueError):
        priority = 0.0
    if priority >= 0.80:
        return 3
...
def _s5_retain_passes_concrete_weapon_contract(value: Mapping[str, Any]) -> bool:
    """Honor S5's explicit weapon, naming and scientific-plausibility judgement.
    ...
    support-dependent or not a concrete direct weapon prevents retention.
    """

    # ``codename_or_metaphor_explainable`` is conditional on the naming
    # style.  The reviewer is required to emit this boolean for every row,
    # and commonly returns ``false`` for ordinary physical-form names (for
    # example, “环形碎片束拦截弹”) precisely because they are *not* codenames.
    # Treating that value as a universal gate silently rejects the entire
    # S5 pool and produces directions=0.  Only enforce it when a codename or
    # metaphor was actually chosen.
    naming_style = str(value.get("naming_style", "")).strip().lower()
    codename_check_failed = (
        value.get("codename_or_metaphor_explainable") is False
        and (
            not naming_style
    ...
    return not (
        ...
        or (
            value.get("naming_semantics_aligned") is False
            and not naming_repaired
        )
...
def _s5_portfolio_fallback_result(
    ...
    *,
    ...
    for item in candidates:
        direct_effects = [
            str(value).strip()
            for value in getattr(item, "direct_military_effects", [])
            if str(value).strip()
        ]
        equipment_forms = [
            str(value).strip()
            for value in getattr(item, "equipment_forms", [])
            if str(value).strip()
        ]
        ...
        spine_count = sum(
            bool(str(getattr(item, field, "") or "").strip())
    ...
    rows.sort(
        key=lambda row: (
            -row[1],
            -row[2],
            str(getattr(row[0], "hypothesis_id", "")),
        )
    )
    selected = rows[: max(1, int(maximum))]
    selected_ids = {
        str(getattr(item, "hypothesis_id", "")) for item, _spine, _score in selected
    }
    decisions: list[dict[str, Any]] = []
    for item, spine_count, _score in rows:
        hypothesis_id = str(getattr(item, "hypothesis_id", ""))
        frontier = str(getattr(item, "frontier_principle", "") or "").strip()
        discontinuity = str(
            getattr(item, "technology_discontinuity", "") or ""
        ).strip()
        disruptive_difference = str(
            getattr(item, "core_disruptive_difference", "") or ""
        ).strip()
        disruptive_shift = str(
            getattr(item, "disruptive_shift", "") or ""
        ).strip()
        changed_variable = str(
            getattr(item, "changed_confrontation_variable", "") or ""
        ).strip()
        ...
        innovation_basis = (
            "创新性={innovation:.2f}；需求性={demand:.2f}；科学可行性={feasibility:.2f}；"
            "效能性={effectiveness:.2f}；发展性={development:.2f}；依据：{basis}"
        ...
        decisions.append(
            {
                ...
                "reason": (
                    "S5模型暂不可用；按S3/S4已写入的直接装备与语义脊柱保守保留。"
                    if hypothesis_id in selected_ids
                    else "S5模型暂不可用；容量降级仅保留语义脊柱更完整的直接作战候选。"
                ),
                ...
                "displaced_operational_mode": str(
                    getattr(item, "original_paradigm", "") or ""
                ).strip()[:500],
                ...
                "final_name": str(getattr(item, "title", "") or "").strip(),
                "name_changed": False,
                "naming_style": str(getattr(item, "naming_style", "") or "").strip(),
                "naming_anchor": str(
                    getattr(item, "core_disruptive_difference", "") or ""
                ).strip()[:240],
    ...
    return {
        ...
        "portfolio_summary": (
            "S5模型不可用，按S3/S4已提交的直接装备身份、制胜逻辑和语义脊柱完成保守排序；"
            "未补写候选属性。"
        ),
...
def _compact_s6_authored_card_event(direction: Mapping[str, Any]) -> dict[str, Any]:
    ...
    keep = (
        "hypothesis_id", "card_binding_id", "name", "source_hypothesis_title", "type",
        ...
        "disruption_tier", "displaced_operational_mode", "new_operational_mode",
        "winning_relation_shift", "innovation_basis",
    )
    return {
...
def _quality_cluster_candidate_instruction() -> str:
    ...
    return (
        ...
        "制胜关系和直接战果，不复述字段或套统一句式；直接从关键机制或作战条件起笔，禁止用"
        "“这是一种”“这是一型”“它是”“一种”等定义式套话开头，避免连续堆叠‘凭借、从而、形成’。"
        "优先写成自然完整的‘机制或条件→直接战果’短句。面向普通专业读者，除公认且不可替代的"
        "正式名称外不使用无解释的英文缩写或字母简称；专业军语和高级技术词可以使用，但必须放在"
        "完整因果句中，不能靠术语堆叠代替逻辑。naming_rationale说明整装命名理由，不得逐词拆解。"
        + QUERY_SPECIFIC_WEAPON_NAMING_CONVENTION
...
def _parallel_s6_card_instruction() -> str:
    ...
    return (
        ...
        "输入严格只有query_semantics、candidate_weapon、winning_logic_overview三项；expert_review_feedback仅作可解释提示，不能覆盖当前Query、冻结身份、证据或科学约束，也不得要求或臆造S5指标、分类、证据、验证或其他卡信息。"
        "候选身份是写作锚点，不得换成另一装备；你拥有本卡场景推演、技术论证、作战流程、能力分类和文字编辑权。先识别被淘汰的旧作战模式、本装备创造的新作战模式，以及指挥权、兵力组织、时空存在、接敌几何或攻防交换的改写，再推演专属作战链与可实现本体，再直接写成决策短卡。"
        "同一次模型调用内部完成两遍工作：先写五栏初稿，再以总编辑视角静默复核并直接返回唯一终稿：概述、技术实现、作战流程、能力效果、制胜逻辑；不得输出初稿、提纲或审校过程。首轮以高军事价值因果为先，五栏各自默认落在约350至400个中文字的软窗口，尽量一次自然成稿；可用3至5个自然句群承载完整因果，但不得套固定句式。若静默计量发现某栏明显低于约300字，应优先在本次成稿内补入该栏独有的约束、动作、状态、战果或交换关系，尽量不要依赖事后补写；若已闭环则不为长度硬扩；若明显超过约350字，删去重复背景和次要铺陈。兼容参考仍记作‘五栏每栏均以自然完整的350至400个中文字为软编辑目标’、‘五栏合计约1750至2000字’，但不是硬长度，不追求精确计数或截断，不因篇幅偏差失败、重试、截断或机械扩写，不为凑字重复。"
        "审校五栏是否完整、独立因果、装备本体闭合、专属流程、新任务和旧规则改写；缺约束、动作、状态、战果或交换关系就在本稿补足，不留给后续补写，闭环栏目不扩写。检查跨栏是否重复背景、结论或句式；删除跨栏重复、删除重复背景、结论、句式和空泛智能化标签。"
        "概述写真实未来作战场景、旧模式失效、新模式、被颠覆的制胜方式和直接战果；技术明确1至2项主攻关键技术及各自最核心的技术痛点，讲清难在哪里、受什么物理或工程约束、为何现有做法不足、原理如何落装，交代必要的落装位置、作用对象和关键耦合，并说明技术断点为何使新模式成为可能；不展开样机、半实物、对抗试验或判退条件。"
        "流程只写能否打仗、能否打胜仗的专属节点：敌我态势、行动主体、进入条件、关键动作、任务状态变化、转入下一节点的条件、拒打/终止和战果续接；不要套发现—决策—打击—评估骨架。效果写过去无法执行的新任务或新场景、目标、压力、直接战果、持续影响和敌方行动约束，能力分类必须显示在画像开头。"
        "技术面向一线研发人员，使其判断主攻方向与可实现性；不用成熟度标签、热门技术或组件清单代替判断。制胜逻辑突出旧作战模式、交换关系重写与新制胜作战模式，说明旧模式的局限、关键关系如何改变，以及由此形成的未来作战制胜与制衡方式及成立条件。直接围绕本装备最有价值的判断自然撰写，不把这些概念写成固定开场白或逐项提示词，不重复背景或堆砌口号。"
...
S6_PORTRAIT_MIN_AVERAGE_EFFECTIVE_CHARS = 300
...
def _s6_portrait_module_lengths(modules: Mapping[str, Any] | object) -> dict[str, int]:
    ...
    if not isinstance(modules, Mapping):
        return {key: 0 for key, _ in CAPABILITY_PORTRAIT_MODULES}
    return {
        key: len(re.findall(r"[\u3400-\u4dbf\u4e00-\u9fff]", str(modules.get(key, ""))))
        for key, _ in CAPABILITY_PORTRAIT_MODULES
    }


def _s6_short_portrait_modules(modules: Mapping[str, Any] | object) -> list[str]:
    """Return repair targets only when the five-module average is below 220."""

    lengths = _s6_portrait_module_lengths(modules)
    average = sum(lengths.values()) / max(1, len(CAPABILITY_PORTRAIT_MODULES))
    if average >= S6_PORTRAIT_MIN_AVERAGE_EFFECTIVE_CHARS:
        return []
    return [
        key
        for key, _ in CAPABILITY_PORTRAIT_MODULES
        if lengths.get(key, 0) < S6_PORTRAIT_MIN_AVERAGE_EFFECTIVE_CHARS
    ]
...
def _parallel_s6_short_module_repair_instruction(
    ...
    labels = {
        key: label for key, label in CAPABILITY_PORTRAIT_MODULES
    }
    short_labels = "、".join(labels.get(key, key) for key in short_module_keys)
    return (
        "你是S6单装备能力画像的独立补写作者，继续使用本卡专属Codex CLI隔离会话。"
        "当前首稿只有以下栏目明显过短："
        + short_labels
        + "。这是首稿异常兜底，不是常规写作步骤；只对这些短栏补充缺失的真实论证链；"
        "其余栏目逐字保持，不改名称、装备身份、"
        "能力分类、置信度、证据或任何组合信息。补写只补缺失的高军事价值因果，通常将受影响栏目收敛到约300至350字；"
        "五栏平均有效中文字达到300即可视为脱离质量增强触发区间，不要求每栏单独达到硬阈值，也不为凑字扩写。"
        "各栏补写重点如下：概述补齐旧模式失效→新模式出现→直接战果→新任务空间；技术栏补齐关键技术的"
        "落装位置、物理或工程约束、传统做法不足及技术断点为何使新模式成为可能；流程栏补齐主体、"
        "进入条件、动作、状态变化与转段或终止条件，并体现部署、授权、协同、交战时序或任务组织的改变；"
        "效果栏区分指标提升与过去无法执行的新任务，补齐直接战果、对敌约束和新场景；制胜栏补齐旧规则、"
        "被逆转的假设、新交换关系和对手被迫改变的组织或防御方式。禁止用颠覆口号代替因果链，也禁止重复原句、同义改写、术语"
        "堆砌或机械凑字。返回完整五栏，并逐字回传card_binding_id和hypothesis_id。只输出"
        "schema规定的严格JSON。"
    )
...
def _parallel_s6_quality_repair_instruction(quality_issues: Sequence[str]) -> str:
    ...
    return (
        "你是S6单装备能力画像的二次编辑作者。首轮已经形成可交付卡片，当前只需做一次"
        "可选质量增强，问题提示如下："
        + issue_text
        + "。只改善画像正文中确实存在的偏短、跨栏重复、模板化表达或论证断点；不得改变名称、"
        "card_binding_id、hypothesis_id、主装备身份、目标、直接战果、分类、证据或流程事实。"
        "按本卡实际语义重写受影响模块，保留未受影响模块；受影响栏目优先收敛到约300至350字，复杂因果未闭合时才自然延展，"
        "重点补足关键因果、工程约束、专属动作、状态变化、直接战果和交换关系；不要为了字数添加空泛背景、技术清单或重复句。"
        "不得删除首稿中已经成立的装备专属判断，不得以修复一个问题为代价引入新的栏目职责缺失、跨栏重复或语义退化。"
        "必须返回完整五栏和原路由字段。若无法确定改进，原样返回首稿也可以。只输出严格JSON。"
    )
...
def _bounded_s6_parallelism(runtime_budgets: Mapping[str, Any] | object) -> int:
    """Resolve the operator setting as a safe cap on profile concurrency."""

    try:
        operator_limit = int(
            os.environ.get("EQUIPMENT_DR_S6_CODEX_CONCURRENCY", "8")
        )
    except (TypeError, ValueError):
        operator_limit = 6
    ...
    return max(1, min(8, operator_limit, profile_limit))
...
def _s6_card_attempt_limit() -> int:
    ...
    try:
        configured = int(os.environ.get("EQUIPMENT_DR_S6_CARD_ATTEMPTS", "2"))
    except (TypeError, ValueError):
        configured = 2
...
async def analyze_winning_subagents(
    ...
    steps = [
        ...
        (
            ...
            "改变的关键前提和直接—间接—最终军事效果；对已知科学只做静默合理性检查，不要求本步骤展开"
            "工程瓶颈、成熟度、参数、验证方案或可证伪失败边界，相关内容留给后续装备论证。"
        ...
        (
            ...
            "直接证据与推导判断，最多6项。每项说明装备对象、作用机理、直接战果、适用边界和证据/推断性质；"
            "科学可实现性只做已知物理自洽检查，不要求S4输出工程瓶颈、成熟度、精确参数、验证方案或可证伪边界。"
            "priority/type/feasibility可以保留粗略暂定判断，但不得用等级代替科学因果；详细工程与验证论证留给后续审查。"
            "禁止用后续S5/S6作为S4推导来源。"
            "若动态专用Agent或S3已经给出优先序，必须保持该顺序；确需调整时逐项说明权重和证据理由。"
            "若填写feasibility_basis，只用一句话说明物理机理与装备本体是否自洽，不另造分级。新增具体装备型号"
            ...
            "复述同一事实，深度留给因果机制。S3每条effect_chain必须在capability_mapping"
        ...
        (
            ...
            "S5复核命名时按装备本体的具体作用和独特锚点判断主导特征：物理形态/结构构型、材料介质、"
            "新物理原理、独特运动、反传统隐喻、数量密度规模，或极少数不可替代的使命身份；"
        ...
        (
            ...
            "semantic_consistency_check用于本卡内部自检和运行时身份审计；它不是关键词命中或机械语义质量门。"
            "若发现字段冲突，应优先在本次成稿内修正；运行时只对主装备身份、卡片绑定、目标、发射域和跨卡污染做确定性安全复核，"
            "不会因某个自检措辞、分类词或本地语义诊断未命中而删除、降级或阻断一张已入选卡。"
            'consistent应输出JSON布尔值true作为模型自检结果，不得输出字符串"true"；该自检结果不替代运行时安全复核。逐卡复核完成后可进行组合级复核：比较全部卡片的主装备、发射域、目标、作用机理和验证指标；发现实质重复或'
            "身份冲突时记录告警并沿用冻结候选，不在S6机械合并、替换或改标题。"
            ...
            + CAPABILITY_EFFECTS_GUIDANCE
            + WINNING_LOGIC_GUIDANCE
            + "各栏不追求精确计数、固定句数或统一句式，首轮通常约300至350字并以高军事价值信号为准；写到本栏独有结论完整即停止。"
    ...
    s6_parallel_authoring_only = bool(
        ...
        and host.supports_agent_runtime
        and (
            host.provider_kind != "codex_cli"
            or getattr(host.provider, "provider_type", "") == "codex_cli"
        )
    )
    ...
    async def execute_dynamic_mission_graph() -> None:
            ...
            if (
                ...
                try:
                    selection_text = await host._run_core_json(
                        ...
                        "面向未来战争打赢需要，提出可供S3/S4自由接受、重构或舍弃的开放制胜关系。"
                        "各提示应改变不同的任务链断点、接敌关系、时空存在、效应交换或对手决策约束，"
                        "不能只是为同一平台更换任务动作；不要命名装备、指定技术路线、分配固定维度或"
                        "裁决候选。只输出少量简短提示，模型失败时控制器会自行继续生成。",
                        {
                            "query": shared.get("topic", ""),
                            "open_angle_seeds": [
                                {
        ...
        async def materialize_active_s3_instances() -> None:
            ...
            emit_swarm_event(
                ...
                rule=(
                    "开放角度选择器仅提供避同构提示；全部有界S3/S4创作容量进入独立Codex调度"
                ),
            )
        ...
        def task_for_instance(instance: WinningAgentInstance) -> SpecialistTask:
            ...
            if instance.mission_node in {"S3", "S4"} and not instance.hypothesis_id:
                ...
                reserved_angles = assignment.get("reserved_other_angles", [])
                reserved_angles = (
                    reserved_angles if isinstance(reserved_angles, list) else []
                )
                occupied_relations = [
                    " / ".join(
                    ...
                    for item in reserved_angles[:6]
                    if isinstance(item, Mapping)
                ]
                diversity_guard = (
                    "已占用的其他关系（仅用于避同构，不要求复述）："
                    + "；".join(occupied_relations)[:900]
                    if occupied_relations
                    else "当前无已占用关系，优先选择Query中尚未被覆盖的未来战争制胜断点。"
                )
                if any(semantic_parts):
                    ...
                    purpose = (
                        ...
                        "战场存在方式和装备身份，并主动避开同池已占用的作用载体、接敌几何、作战时序和"
                        "对手代价组合，再独立完成自然名称与一句制胜说明。面向未来战争需要的多样性来自"
                        "不同制胜断点，不来自同一装备更换代号、载荷或任务前缀。"
                        + diversity_guard
            ...
            *,
            ...
            candidate_semantic_spine = {
                str(item.hypothesis_id): {
                    ...
                    if key not in {"hypothesis_id", "name", "concise_winning_summary"}
                }
                for item in ledger_candidates
            }
            ...
            if instance.mission_node in {"S1", "S2", "S3", "S4"}:
                ...
                common_input = {
                    ...
                    **(
                    ...
                    "portfolio_creative_diversity_goal": (
                        ...
                        "、具体作用和作用对象共同决定应突出独特物理形态、结构构型、新物理原理、"
                        "装备独特运动、反传统隐喻或数量/密度/规模；这是跨候选的"
                        "开放多样性目标，不得仅为换命名风格复制同一装备。每轮具体采用系统随机分配的"
                        "A—O命名类型，同轮候选类型不重复；名称主体尽量为8—12个汉字。不同候选应有"
                        "不同的核心意象、语法节奏与装备身份表达；若名称只像替换了同一"
                        "底名的前缀，或多个名称都采用‘环境/威胁条件＋自证/自校正/复获/续接＋装备后缀’，"
                        "应回到主装备、作用对象和制胜关系重新创作，而不是事后换词。‘断链、强扰、静默、"
                        "智能、双模、多模、蜂群’等通用能力/场景词不得批量充当名称首部；它们通常应放进"
                        "制胜说明，除非已经凝结为该装备独有的物理存在方式。可借鉴“玄磁”超材料隐身巡弋器、"
                        "“玄鸟”远域感知打击器、等离子流控飞行器、仿生扑翼微型侦察打击弹、千节点自主作战集群"
                        "所体现的本体锚点，但不得照抄示例或把示例变成模板。名称无专名或代号时不得给整个名称加引号；"
                        "确有专名或代号时只给专名或代号部分加中文双引号。候选池中的专名/代号型与自然构型、原理、"
                        "运动、规模或描述型名称应近似均衡，专名/代号型不得成为明显多数；不得为了均衡创造同构换皮候选。"
                    ),
                    "isolation_contract": {
                        "raw_other_agent_sessions_visible": False,
                        "may_recruit_child_agent": False,
                ...
                common_input = {
                    "candidate_ledger": {
                        ...
                        # Keep the routed scope explicit even for the global
                        # reviewer.  It makes the full-pool contract
                        # auditable and prevents a provider adapter from
                        # silently treating an empty/partial snapshot as the
                ...
                common_input = {
                    ...
                    "specialist_task": {
                        ...
                        "military_expert_creative_contract": {
                            "role": (
                                "像真实军工论证中的武器装备总师与作战概念专家：先闭合任务对象、接敌几何、"
                                "武器本体、作用载体、部署时序、直接战果和敌方适应，再命名；名称不是先验答案。"
                            ),
                            "diversity": (
                                "本会话只是独立创作池中的一个席位。主动避开其他席位已经占用的常见平台、"
                                "作用载体和接敌关系；可以提出完全不同的空中、海上、地面、跨介质、定向能、"
                                "可消耗单元或其他物理武器，但不得为了覆盖类别机械凑数。"
                            ),
                            "naming_reference": (
                                "每次创作调用会从A—O命名体系随机分配命名类型；按候选顺序采用相应类型，"
                                "同时保留具体武器本体。类型是名称表达约束，不是装备类别配额或固定词库。"
                                "名称主体尽量控制为8—12个汉字，类型字母和类型名不得写入名称。"
                            ),
                        },
                    },
                    "query": str(shared.get("topic", "")),
            ...
            if instance.mission_node in {"S1", "S2"} and not instance.hypothesis_id:
                ...
                output_schema: dict[str, Any] = {
                    "hypotheses": [
                        {
                            "name": "按随机分配的第1种A—O类型命名、主体尽量8—12个汉字且可识别主装备身份",
                        ...
                        {
                            "name": "按随机分配的第2种A—O类型命名、主体尽量8—12个汉字的另一种主装备名称",
                            "concise_winning_summary": (
                                "与第一项拥有不同武器本体、作用原理或接敌几何的另一条制胜逻辑；"
                                "同构变体不要填写"
                            ),
                        },
                    ],
                ...
                instruction = (
                    task.purpose
                    + load_dynamic_winning_prompt(instance.mission_node)
                )
                    ...
                    "decisions": [
                        {
                            ...
                            "reason": "one concise innovation judgement",
                            "independence_basis": "what makes the candidate materially distinct from other candidates",
                            "innovation_priority": "0..1; how strongly it creates a new-quality combat capability",
                            "dimension_scores": {
                                "innovation": "0..1",
                                "demand": "0..1",
                                "feasibility": "0..1",
                            ...
                            "innovation_basis": "specific changed equipment, principle, engagement relation, effect or exchange logic",
                            "disruption_tier": "paradigm_disruption|new_quality_breakthrough|significant_innovation|incremental_upgrade",
                            "material_innovation_breakpoint_present": "boolean",
                            "ordinary_upgrade_or_function_packaging": "boolean",
                            "displaced_operational_mode": "the conventional combat mode or winning method displaced by this candidate",
                            "new_operational_mode": "the new combat mode made possible by this candidate",
                ...
                instruction = (
                    "你是S5创新颠覆性评审。S5只做候选的创新/新质/颠覆判断，不改名、不补写候选、不审查证据、成熟度、成本、"
                    "工程参数、验证方案或能力画像。输入候选主表只有hypothesis_id、name、concise_winning_summary，另有可选semantic_spine；"
                    "用它理解候选的装备本体、作用原理、改变的交战关系和直接战果。对每个候选只判断：相对传统方案是否存在实质创新断点，"
                    "是否形成新的作战方式或敌我交换关系，是否与其他候选保持独立。普通改型、同一装备换代号/换任务前缀、仅增加智能化或性能参数、"
                    "以及只提供感知/中继/指挥的支援节点，应判reject；真正改变装备本体、作用原理、接敌几何、时空存在、毁伤方式、成本交换或决策权的候选优先retain。"
                    "名称只作为身份线索，名称好坏不能成为创新判断的替代条件，也不能因为名称暂时生硬而淘汰有实质新质机理的候选。"
                    "每条返回decision、reason、independence_basis、innovation_priority、innovation_basis、disruption_tier，必要时补充旧模式→新模式→制胜关系改写。"
                    "incremental_upgrade一律reject；significant_innovation只有形成独立新任务或新作战方式时才可retain。"
                    "不设入选下限，不为凑数回收普通方向；最多保留创新性最强的7条，并按颠覆层级和创新优先级排序。必须对allowed_hypothesis_ids中的每个候选恰好返回一条decision，严格只输出JSON。"
                )
                if instance.archetype != "independent_portfolio_reviewer":
                    raise RuntimeError(
                ...
                output_schema = {
                    "decisions": [
                        {
                            ...
                            "weighted_score": "0..1; runtime recomputes from the five scores",
                            "innovation_basis": "五项分数与综合依据；格式：创新性=0.00；需求性=0.00；科学可行性=0.00；效能性=0.00；发展性=0.00；再写一句材料创新断点",
                            "disruption_tier": "paradigm_disruption|new_quality_breakthrough|significant_innovation|incremental_upgrade",
                    ...
                    "portfolio_order": ["exact retained candidate ids, best first"],
                    "portfolio_summary": "one short selection summary",
                    "stop_reason": "string",
                }
                instruction = (
                    "你是S5独立组合评审，只做候选的创新颠覆性综合评分与组合筛选，不改名、不补写装备字段。"
                    "只做创新性、颠覆性和候选独立性筛选；"
                    "输入候选严格只有hypothesis_id、name和concise_winning_summary，另有同一候选对应的semantic_spine冻结语义脊柱以及Query边界；"
                    "不得要求或使用Query之外的证据、成熟度或工程审查材料；"
                    "语义脊柱用于理解主装备身份、前沿原理、技术断点、核心颠覆差异、作战关系改写和直接战果。"
                    "对每项按五项评分：创新性30%（技术路径、装备形态、作战理念的新颖度与突破潜力）；需求性30%（是否回应Query对应的现实威胁、任务需求和能力短板）；科学可行性20%（原理是否遵守已知科学并能落到武器本体，只做科学判断）；效能性10%（直接战果、使用效率和体系价值）；发展性10%（技术拓展、持续升级和未来应用潜力）。"
                    "五项分数均为0至1，必须写入innovation_basis开头，严格采用‘创新性=0.00；需求性=0.00；科学可行性=0.00；效能性=0.00；发展性=0.00；’格式，再补一句材料创新断点与综合依据；运行时会按五项分数重新计算weighted_score，防止算术误差。"
                    "不得仅因名称暂时生硬、缺少代号解释或缺少工程字段淘汰有实质机理的候选；但若名称无法识别"
                    "唯一可部署武器本体，或concise_winning_summary没有具体作用对象、接敌方式和直接战果，"
                    "应要求回到S3/S4修正或判reject。不要审查证据、成熟度、成本、参数、验证或能力画像。"
                    "逐条判断它是否是相对独立、能直接形成战果的新质武器命题：普通性能加码、把现有功能智能化、流程提速、增程、支援节点包装或同构换皮应reject；"
                    "真正改变装备本体、作用原理、接敌关系、毁伤方式、战场时空存在或成本交换并形成新战果的候选应retain。"
                    "只需返回精简字段：decision、reason、independence_basis、innovation_priority、dimension_scores、weighted_score、innovation_basis、disruption_tier、"
                    "displaced_operational_mode、new_operational_mode、winning_relation_shift；"
                    "若两个候选本质同构，可用decision=merge并填写merge_target_hypothesis_id。"
                    "innovation_priority须体现颠覆幅度：重构本体、原理、接敌关系、直接效应、时空存在或数量级交换通常为0.80以上；weighted_score按五项权重决定最终排序。"
                    "显著但仍沿用主要范式为0.65至0.79；普通改进不得高于0.60。"
                    "不设入选下限，最多保留创新性最强的7条，禁止为凑数回收已经判退的普通方向；portfolio_order只列retain候选。严格只输出JSON。"
                )
            emit_swarm_event(
            ...
            # S3/S4 get a bounded self-critique loop.  Each pass is still an
            # isolated Codex CLI call owned by the creative node; no local
            # template or keyword scorer manufactures a candidate.  The
            # second pass sees only the previous draft and is asked to keep,
            # strengthen, or replace it after a fresh counterfactual check.
            ...
            try:
                configured_creative_passes = int(
                    # Three creative passes balance genuine model self-critique
                    # with the ten-seat dynamic swarm budget: 4 S1/S2 calls,
                    # 30 S3/S4 calls, one angle selector and one S5 review.
                    # Dynamic swarm uses repeated isolated Codex calls as its
                    # invention engine. Four passes give each seat room to
                    # discard a merely polished first draft and discover a
                    # different weapon ontology, while the ledger still
                    # bounds the retained pool.
                    os.environ.get(
                        "EQUIPMENT_DR_S3_S4_CREATIVE_PASSES",
                        str(swarm_controller.policy.get("s3_s4_creative_passes", 3)),
                    )
                )
            except (TypeError, ValueError):
                configured_creative_passes = 3
            creative_passes = (
                max(1, min(4, configured_creative_passes))
                if instance.mission_node in {"S3", "S4"} and not instance.hypothesis_id
                else 1
            )
            for creative_iteration in range(1, creative_passes + 1):
                ...
                if instance.mission_node in {"S3", "S4"} and not instance.hypothesis_id:
                    iteration_input["random_naming_style_assignment"] = _s3_s4_naming_assignment(
                        "|".join(
                            (
                                str(shared.get("run_id", "")),
                                str(shared.get("topic", "")),
                                instance.instance_id,
                                str(creative_iteration),
                            )
                        ),
                        count=2,
                    )
                    iteration_input["combat_realism_naming_contract"] = {
                        "must_identify": [
                            "唯一主装备本体",
                            "发射或释放域及接敌方式",
                            "主要作用对象",
                    ...
                    iteration_system += (
                        "本轮必须遵循random_naming_style_assignment：随机类型与候选位置一一对应；"
                        "名称主体尽量8—12个汉字，并保持具体武器装备本体清晰。"
                        "同时严格执行combat_realism_naming_contract：名称、武器本体、接敌方式、"
                        "作用对象、直接战果和摘要中的作战阶段、敌方反制必须彼此一致；只会制造抽象系统"
                        "或好听代号而不能部署交战的候选应返回空hypotheses。"
                    )
                if creative_history:
                    iteration_input["previous_creative_draft"] = creative_history[-1]
                    # Show the whole small pool from this isolated creator,
                    # not only its last draft. This lets the model deliberately
                    # replace a near-duplicate with a different weapon
                    # architecture while keeping the handoff bounded.
                    ...
                    iteration_system += (
                        "上一轮草稿只作为可被推翻的内部草案。请重新从Query出发做一次反事实审查："
                        "如果它只是现役装备增程、智能化、换载荷、支援节点或机械拼名，必须推倒重来；"
                        "如果核心机理成立，也要检查是否还有更能改变未来战场交战关系的装备构型。"
                        "session_candidate_pool列出本会话已经提出的候选；后续轮次应优先提出至少一个真正不同的装备架构，"
                        "除非你能明确证明新方案改变了装备本体、"
                        "作用原理、接敌几何、时空存在或成本交换，否则不要重复其中任何方案。本轮可返回最终保留的1—2项候选，"
                        "仅当两项真正独立时返回两项，不解释审查过程。"
                    )
                    # Keep the governed S3/S4 phase identity stable so
                    # provider routing, metrics and resume semantics remain
                    # owned by the creative node rather than a new role.
                    iteration_phase = phase
                try:
                    ...
                    # must not erase a valid first creative draft.
                ...
                if instance.mission_node in {"S3", "S4"} and not instance.hypothesis_id:
                    iteration_rows_for_event = parsed_iteration.get("hypotheses", [])
                    if not isinstance(iteration_rows_for_event, list):
                        iteration_rows_for_event = []
                    # Keep one audit receipt for each actual creative Codex
                    # call.  The historical event name remains for API
                    # compatibility; the iteration field distinguishes later
                    # self-critique passes from any separate pre-commit gate.
                    emit_swarm_event(
                        ...
                        candidate_count=min(2, len(iteration_rows_for_event)),
                        candidate_maximum_per_session=2,
                        creative_iteration=creative_iteration,
                        separate_precommit_model_call=False,
                        rule=(
                            "每次S3/S4 Codex创作调用均完成一次自审收据；继续迭代用于比较"
                            "更强新质装备，不构成独立预提交门。"
                        ),
                    )
                    ...
                    usable_rows = [
                        dict(row)
                        for row in iteration_rows[:2]
                        if isinstance(row, Mapping)
                        and str(row.get("name", "") or row.get("title", "")).strip()
                        and str(row.get("concise_winning_summary", "")).strip()
                    ]
                    if usable_rows:
                        # Repeated Codex passes are allowed to replace a weak
                        # draft or introduce a genuinely different weapon.
                        # Keep one latest row per model-authored name so the
                        # session contributes a small, diverse pool rather
                        # than only its final prose revision.
                        ...
                        pool_cap = int(
                            swarm_controller.policy.get(
                                "s3_s4_candidate_pool_maximum_per_session", 2
                            )
                        )
                        ...
                        emit_swarm_event(
                            "winning_s3_s4_creative_iteration_completed",
                            actor=instance.instance_id,
                            graph_id=graph.graph_id,
                            mission_node=instance.mission_node,
                            iteration=creative_iteration,
                        ...
                        # Do not stop merely because the first draft is
                        # syntactically clean.  S3/S4 are the creative
                        # engine, so every configured pass gets a fresh
                        # counterfactual comparison against the prior draft.
                        # The next Codex call may keep, strengthen, or replace
                        # it; local language checks are diagnostics only and
                        # must never become an innovation stopping rule.
                        if not _s3_s4_rows_need_creative_retry(usable_rows):
                            emit_swarm_event(
                                "winning_s3_s4_creative_quality_passed_continue",
                                actor=instance.instance_id,
                                graph_id=graph.graph_id,
                                mission_node=instance.mission_node,
                                iteration=creative_iteration,
                                remaining_iterations=max(
                                    0, creative_passes - creative_iteration
                                ),
                                reason=(
                                    "首稿语言和结构已合格，但继续调用Codex比较是否存在更强"
                                    "的新质装备与制胜关系；不以句式合格提前停止。"
                                ),
                            )
            ...
            if instance.mission_node in {"S3", "S4"} and not instance.hypothesis_id:
                ...
                if name_failures:
                    emit_swarm_event(
                        "winning_s3_s4_name_authoring_retry_required",
                        actor=instance.instance_id,
                        graph_id=graph.graph_id,
                        mission_node=instance.mission_node,
                        ...
                        rule=(
                            "名称出现明显占位/非最终标记；交由下一次独立Codex创作重命名，"
                            "不在本地替换、不套用固定词库。"
                        ),
                    )
                if language_failures or name_failures:
                    repair_focus = (
                        "；同时，name_failures中的名称含有明显占位或非最终标记。对这些候选必须基于"
                        "同一装备本体、作用原理、作用对象和直接战果原创重命名，名称要像真实新研军事装备；"
                        "不得用占位符、待定、示例、测试或英文placeholder，也不要从固定词库拼接。"
                        if name_failures
                        else ""
                    )
                    revision_text = await host._run_core_json(
                        runtime_agent_id,
                        (
                            _creative_s3_candidate_instruction()
                            + "上一稿中部分制胜概述或装备名称未通过写作质量门。保持候选的装备本体、"
                            "作用原理、作用对象和直接战果不变；"
                            + ("对名称失败项允许并要求重写name，其他名称保持不变；" if name_failures else "保持每个候选的装备身份和名称不变；")
                            + "只对issues所指概述进行整句重写；不要用词表替换或局部拼补。直接从关键条件或"
                            "机理起笔，完整说明改变的对抗关系与直接战果。专业术语允许保留；无解释英文缩写"
                            "必须改为中文全称或在首次出现时给出中文全称。"
                            + repair_focus
                            + "返回全部候选并保持顺序。"
                        ),
                        {
                        ...
                        output_schema,
                        task.max_output_tokens,
                        phase="winning_swarm_dynamic_seed_language_revision",
                    )
                    revised_result = _parse_json_object(revision_text)
                    revised_rows = revised_result.get("hypotheses", [])
                    if isinstance(revised_rows, list) and len(revised_rows) == len(
                        raw_hypotheses
                    ):
                        preserved_rows: list[dict[str, Any]] = []
                        for original, revision in zip(
                            raw_hypotheses, revised_rows, strict=True
                        ):
                            if not isinstance(original, Mapping):
                                continue
                            revised_summary = (
                                str(
                                    revision.get("concise_winning_summary", "")
                                ).strip()
                                if isinstance(revision, Mapping)
                                else ""
                            )
                            original_name = str(
                                original.get("name", "") or original.get("title", "")
                            ).strip()
                            revised_name = str(
                                revision.get("name", "") or revision.get("title", "")
                            ).strip() if isinstance(revision, Mapping) else ""
                            original_name_failed = bool(
                                _s3_s4_name_authoring_issues(original_name)
                            )
                            preserved_rows.append(
                                {
                                    **dict(original),
                                    "name": (
                                        revised_name
                                        if original_name_failed and revised_name
                                        else original_name
                                    ),
                                    "concise_winning_summary": (
                                        revised_summary
                                        or str(
                                            original.get("concise_winning_summary", "")
                                        ).strip()
                                    ),
                                }
                            )
                        result = {**result, "hypotheses": preserved_rows}
                        raw_hypotheses = preserved_rows
                    emit_swarm_event(
                        "winning_candidate_summary_language_revision_completed",
                        actor=instance.instance_id,
                        graph_id=graph.graph_id,
                        candidate_count=len(raw_hypotheses),
                        initial_failure_count=len(language_failures),
                    )
            emit_swarm_event(
        ...
        def compact_candidate_ledger_for_review() -> None:
            ...
            if str(swarm_controller.policy.get("policy_id")) == "winning_swarm_dynamic_v2":
                # Dynamic S5 receives the complete drained S3/S4 ledger in one
                # full-pool call; never pre-trim it with a local quota.
        ...
        while pending or running_instances:
            ...
            # S3/S4 branches are published as they complete, but S5 is not
            # allowed to review a partial pool.  Its static dependencies on
            # every creator form one explicit full-pool calibration barrier;
            # all creative Codex calls remain parallel before that barrier.
            if (
                ledger is not None
                ...
                # Do not pre-merge or pre-trim the dynamic candidate pool.
                # The single S5 Codex call must calibrate every original
                # name-and-winning-logic pair together.
            ...
            for completed_call in done:
                ...
                if isinstance(outcome, BaseException):
                    ...
                    if instance.mission_node == "S5" and ledger is not None:
                        # S5 is a judgement layer.  If its isolated CLI is at
                ...
                if instance.mission_node in {"S1", "S2"} and not instance.hypothesis_id:
                    ...
                    if incremental_ids:
                        ...
                        # Retain bookkeeping for audit compatibility. No
                        # semantic pre-merge or incremental S5 call is
                        # scheduled: the static reviewer sees the complete
                        # original pool after all S3/S4 producers finish.
                    ...
                    if instance.archetype == "independent_portfolio_reviewer":
                        ...
                        required_ids = set(candidate_scope) or set(known_ids)
                        ...
                        if duplicate_ids or missing_ids:
                            ...
                            raw_decisions = [decisions_by_id[item] for item in sorted(required_ids)]
                            emit_swarm_event(
                        ...
                        for raw in raw_decisions:
                            ...
                            disruption_tier = str(
                                raw.get("disruption_tier", "") or ""
                            ).strip().lower()
                            if disruption_tier:
                                candidate_disruption_tiers[source_id] = (
                                    disruption_tier
                                )
                            displaced_mode = str(
                            ...
                            candidate_innovation_bases[source_id] = str(
                                raw.get("innovation_basis", "")
                            ).strip()[:500]
                                ...
                                swarm_rejections.append(
                                    {
                                        ...
                                        "stage": "full_pool_s5_portfolio_review",
                                    }
                                )
                            ...
                            emit_swarm_event(
                                ...
                                dimension_scores=candidate_dimension_scores.get(source_id, {}),
                                weighted_score=candidate_weighted_scores.get(source_id, 0.0),
                                disruption_tier=disruption_tier,
                        ...
                        if not retained_ids and review_rows_by_id:
                            rescue_rows = [
                                ...
                                if str(row.get("decision", "")).strip().lower()
                                == "retain"
                            ]
                            rescue_rows.sort(
                                key=lambda item: (
                            ...
                            rescue_ids = [
                                candidate_id
                                for candidate_id, _ in rescue_rows[:7]
                            ]
                            if rescue_ids:
        ...
        if ledger is None:
            ...
            compact_candidate_ledger_for_review()
        # The dynamic portfolio is the direct result of the complete S3/S4
        # pool plus one full-pool S5 retain/merge/reject calibration. There is
        # no second
        # expert score, evidence/TRL audit, repair wave, or completeness ranker.
        # S5's innovation judgement is a prioritisation signal only: it orders
        # all retained independent candidates and never creates a one-per-family
        # rule or a fixed target count.
        maximum = int(swarm_controller.policy.get("finalist_maximum", 12))
        ordered_hypotheses = sorted(
            ledger.hypotheses,
            key=lambda item: (
                -candidate_weighted_scores.get(item.hypothesis_id, 0.0),
                -_s5_disruption_tier_rank(
                ...
                portfolio_order_hints.get(item.hypothesis_id, 10**9),
                -float(item.score or 0.0),
                item.hypothesis_id,
            ),
        )
        selected_ids_ordered = [
            item.hypothesis_id for item in ordered_hypotheses[:maximum]
        ]
        ...
        def capability_direction(
            ...
            card = {
                ...
                "validation_plan": list(item.validation_plan[:8]),
                "future_trigger": item.technology_horizon,
                "failure_boundary": "；".join(item.failure_boundaries[:2]),
                ...
                "s5_weighted_score": candidate_weighted_scores.get(
                    item.hypothesis_id, 0.0
                ),
                "disruption_tier": candidate_disruption_tiers.get(
                    item.hypothesis_id, ""
                ),
        ...
        accumulated["winning_swarm"] = {
            ...
            "portfolio_quality_gate": {
                ...
                "selection_rule": (
                    "多选制：增量五轴语义聚类与S5独立组合评审决定保留、合并和判退；"
                    "凡通过完整Query因果、直接战果与独立性审查者均可入选，不实行一类一条；"
                    "组合内部按创新/新质能力优先排序，数量仅受S6并行成稿容量约束"
                ),
    ...
    async def generate_parallel_s6_cards(
        *,
        ...
        portrait_modules_schema = {
            "overview": (
                "通常用约350至400个中文字精简阐述，突出最重要的新质判断；同时交代真实未来作战场景与阶段、旧作战模式为何不再成立、本装备如何创造新作战模式、"
                "颠覆何种制胜方式、形成什么直接战果及打开什么新任务空间；明确被改写的指挥权、兵力组织、时空存在、接敌几何或"
                "核心交换关系，以及对手被迫承担的代价或选择；"
                "不得只写场景、功能、指标、技术清单或流程复述"
            ),
            "technology_implementation": (
                "通常用约350至400个中文字精简阐述：面向一线研发人员，明确决定装备成败的1至2项"
                "核心研发技术，并分别讲清最核心的技术痛点、物理或工程约束及现有做法为何难以满足；交代"
                "关键原理、必要的落装位置、作用对象、控制关系、关键耦合与必要接口，并说明技术断点为何使过去不可行的新作战模式成为可能；使研发人员能判断主攻方向和可实现性；不要求展开完整实现路线、"
                "输入输出或全部接口，不写组成介绍、研发流程、验证计划或零部件清单"
            ),
            "operational_process": (
                "通常用约350至400个中文字精简阐述真实未来战场装备专属关键节点链：围绕能打仗、打胜仗选择决定成败的节点，每个节点自然包含敌我态势、行动主体、进入条件、"
                "关键动作、状态变化、转段/拒打/终止条件和战果续接；体现部署、授权、协同、交战时序或任务组织相对旧模式的改变，"
                "不得套用通用观察—判断—决策—行动链"
            ),
            "capability_effects": (
                "通常用约350至400个中文字精简阐述：区分普通指标提升与过去无法执行的新任务，写清针对何种目标、在何种对抗压力下形成何种新质作战能力、直接战果、"
                "持续战场影响、对敌方的制约和消耗关系及由此出现的新场景，帮助作战人员理解装备的颠覆性作战价值；"
                "不复述流程"
            ),
            "winning_logic": (
                "约350至400个中文字。" + WINNING_LOGIC_GUIDANCE
            ),
        }
        ...
        card_direction_schema["card_binding_id"] = (
            "逐字回传candidate_weapon.card_binding_id；不得修改、遗漏或替换"
        )
        card_direction_schema["hypothesis_id"] = (
            "逐字回传candidate_weapon.hypothesis_id；不得修改、遗漏或替换"
        )
        ...
        for brief in briefs:
            ...
            if not hypothesis_id:
                continue
            brief["card_binding_id"] = "s6-card-" + sha256(
                f"{binding_scope}:{hypothesis_id}".encode("utf-8")
            ).hexdigest()[:20]
        if not briefs:
            limited_result = {
        ...
        card_semaphore = asyncio.Semaphore(
            max(1, min(8, int(handoff.get("s6_parallelism", 8) or 8)))
        )
        ...
        def limited_card_from_brief(
            ...
            fallback_modules = parse_capability_portrait_modules(
            ...
            if fallback_modules:
                ...
                if isinstance(classification, Mapping) and classification:
                    fallback_modules["capability_classification"] = dict(
                        classification
                    )
                direction["capability_portrait_modules"] = fallback_modules
            ...
            direction["s6_authoring_quality_warnings"] = [
                "独立模型写卡与错峰恢复均失败；已仅使用入选装备的冻结语义组装保底能力卡，等待定向续写"
            ]
            draft = str(
                direction.get("concise_winning_summary")
                or direction.get("capability_outcome")
        ...
        if isinstance(prior_directions, list):
            ...
            for brief in briefs:
                ...
                if dynamic_s6_authoring:
                    ...
                    if str(
                        prior_direction.get("_s6_authoring_input_fingerprint", "")
        ...
        async def author_card(
            ...
            *,
            ...
            base_card_instruction = (
                load_dynamic_winning_prompt("S6")
                if dynamic_s6_authoring
                else _parallel_s6_card_instruction()
            )
            emit_swarm_event(
                ...
                status="running",
                attempt=attempt,
                provider_timeout_seconds=_s6_card_timeout_threshold(
                    resume=resume_s6_only
                ),
            ...
            async with card_semaphore:
                ...
                card_text = await host._run_core_json(
                    ...
                    base_card_instruction
                    + (
                        " 这是本卡的独立重试。上次未形成可验收成稿，原因是："
                        + retry_reason[:240]
                        + "。请重新完整成稿并重点检查该缺陷；不要解释重试过程。"
                        if retry_reason
                        else ""
                    ),
                    card_payload,
                    ...
                    5200 if dynamic_s6_authoring else 3200,
                    phase=(
            ...
            authoring_input_fingerprint = (
                _dynamic_s6_input_fingerprint(card_payload)
                if dynamic_s6_authoring
                else ""
            )
            ...
            portrait_issues = capability_portrait_quality_issues(authored_modules)
            repair_issues = list(portrait_issues) if initial_short_modules else []
            # First-pass authoring owns normal length, style and density
            # correction.  When the five-module average remains at or above
            # 220, every local diagnostic is advisory only: do not start a
            # second model merely to polish wording or pull one column toward
            # the 300-350 soft window.  A repair remains available only for an
            # abnormally thin/incomplete first pass, and it is transactional.
            if portrait_issues and not initial_short_modules:
                quality_warnings.extend(portrait_issues)
            # Card enhancement is a best-effort second model call.  Operators
            # may disable it for slow/reasoning-heavy gateways; the complete
            # first pass remains authoritative and is never treated as a
            # failure merely because enhancement was skipped.
            enhancement_enabled = str(
                os.environ.get("EQUIPMENT_DR_S6_CARD_ENHANCEMENT", "1")
            ).strip().lower() not in {"0", "false", "no", "off"}
            if repair_issues and enhancement_enabled:
                first_pass_direction = dict(direction)
                first_pass_modules = dict(authored_modules)
                try:
                    # Repairs use the same per-card semaphore as first-pass
                    # authoring. Without this guard, a full wave of short
                    # cards could launch an unbounded second CLI burst after
                    # the first wave, recreating the very contention that
                    # caused partial S6 failures.
                    async with card_semaphore:
                        repair_call = host._run_core_json(
                            ...
                            {
                                **card_payload,
                                "current_direction": first_pass_direction,
                                "quality_issues": repair_issues[:8],
                                "short_module_keys": initial_short_modules,
                            },
                            {
                                "direction": active_card_direction_schema,
                                "capability_image_draft": "二次增强后的该装备能力画像单句结论",
                            },
                            4200 if dynamic_s6_authoring else 3200,
                    ...
                    if required_module_keys <= set(repaired_modules) and all(
                        str(repaired_modules.get(key, "")).strip()
                        for key in required_module_keys
                    ):
                        # Only prose modules are transactional. Every other
                        # field remains from the first pass, so an imperfect
                        # repair can never mutate identity or evidence.
                        repaired_portrait = assemble_capability_portrait_modules(
                            repaired_modules
                        )
                        repaired_issues = capability_portrait_quality_issues(
                            repaired_modules
                        )
                        identity_probe = {
                            **direction,
                            "capability_portrait_modules": repaired_modules,
                            "capability_portrait": repaired_portrait,
                        }
                        if (
                            len(repaired_issues) < len(portrait_issues)
                            and not _s6_cross_card_identity_issues(
                                identity_probe, briefs
                            )
                        ):
                            direction["capability_portrait_modules"] = repaired_modules
                            direction["capability_portrait"] = repaired_portrait
                            authored_modules = repaired_modules
                ...
                emit_swarm_event(
                    ...
                    module_character_counts=_s6_portrait_module_lengths(
                        authored_modules
                    ),
                ...
                quality_warnings.extend(
                    f"S6二次画像增强已由配置跳过（{len(repair_issues)}项质量提示保留为审计信息）"
                    for _ in [0]
                )
                emit_swarm_event(
            ...
            if authoring_input_fingerprint:
                direction["_s6_authoring_input_fingerprint"] = (
                    authoring_input_fingerprint
                )
            identity_issues = _s6_cross_card_identity_issues(direction, briefs)
            if identity_issues:
                # Run this before the result is cached or merged.  Restoring
                # the frozen title cannot make prose authored for a sibling
                # candidate valid for the current card.
            ...
            # The model's boolean self-report is an audit hint, not a release
            # switch. Runtime has already rebound every frozen identity field
            # and performed the exact cross-card contamination check above.
            # A missing/false self-report therefore schedules review but must
            # not discard otherwise complete, correctly bound authored prose.
            if semantic_check.get("consistent") is not True:
                quality_warnings.append(
                    "模型语义一致性自检未明确通过；运行时已按冻结身份完成确定性复核"
                )
            ...
            # locked above before S6 starts. S6 may make one isolated,
            # card-local semantic expansion call when a first draft contains
            # visibly short modules; it never reruns the portfolio batch and
            # never pads prose locally.
            emit_swarm_event(
        ...
        async def author_card_with_retry(
            ...
            for attempt in range(1, maximum_attempts + 1):
                try:
                    ...
                    # Desynchronize simultaneous provider/gateway failures so
                    # retries do not create a second synchronized storm.
                    await asyncio.sleep(min(1.0, 0.1 * position * attempt))
        ...
        if pending_cards:
            ...
            for (position, brief), card_result in zip(
                ...
                if isinstance(card_result, BaseException):
                    ...
                    # for this card so retries no longer compete with sibling
                    # generations for the same provider/account capacity.
                    emit_swarm_event(
        ...
        if card_quality_warnings:
            result["s6_card_quality_warnings"] = list(
                dict.fromkeys(card_quality_warnings)
            )[:16]
        if card_failures:
            ...
            result["s6_reference_weapons"] = [
                {
                    **dict(item[1]),
                    ...
                    "s6_eligible": True,
                    "reference_reason": (
                        "S6独立原创调用暂不可用，已保留入选身份并用冻结语义形成可见保底卡；"
                        "后续定向恢复只替换画像正文，不重新判退装备。"
                    ),
                }
                for item in authored
            ...
            result["s6_fallback_cards"] = list(result["s6_reference_weapons"])
            # A selected weapon remains a formal S6 card even when the
            # provider is unavailable. The fallback is visibly marked and
            # resumable, but never silently removed from the user's portfolio.
        return result
        ...
        for inner_iteration in range(1, max_inner_iterations + 1):
            ...
            if reuse_s6_without_model:
                ...
                repair_card_semaphore = asyncio.Semaphore(
                    max(1, min(8, int(s6_handoff.get("s6_parallelism", 8) or 8)))
                )
                direction_schema = schema["concept_directions"][0]
                repair_prompt = (
                ...
                try:
                    text = await host._run_core_json(
                        ...
                        + (
                            " random_naming_style_assignment已按concept_directions输出顺序从A—O随机分配"
                            "互不重复的命名类型；逐项采用相应类型，名称主体尽量8—12个汉字，且必须保留"
                            "具体武器装备本体。不得把类型字母或类型名写入名称。"
                            if index in {3, 4}
                            else ""
                        )
    ...
    async def run_physical_cohort(
        ...
        *,
        ...
        if 4 in indices:
            cohort_input["random_naming_style_assignment"] = (
                _s3_s4_naming_assignment(
                    "|".join(
                        (
                            str(shared.get("run_id", "")),
        ...
        text = await host._run_core_json(
            ...
            "适用条件和置信度，禁止虚构精确百分比。若输入包含random_naming_style_assignment，"
            "S3/S4产生的候选或concept_directions须按输出顺序逐项采用随机分配的A—O命名类型，名称主体尽量8—12个"
            "汉字并保留具体武器装备本体，不得在名称中写类型标记。只输出严格JSON。",
            cohort_input,
            ...
            if portrait_module_targets:
                async def repair_portrait_modules(
                    ...
                    repair_text = await host._run_core_json(
                        ...
                        "精确字数、固定句数或统一句式，但必须补足明显过短的论证；只保留该栏独有且改变军事判断的信息。"
                        + CAPABILITY_PORTRAIT_CONCISION_GUIDANCE
                        + "技术栏由Codex结合本装备重新判断关键"
                        "技术痛点，讲清原理、具体实现、解除的限制和工程边界，不套固定技术链或组件清单；流程栏"
                        "落到具体战役/战斗场景和交战时序；效果栏区分指标提升与过去无法执行的新任务，只写战果、能力与验收轴；制胜栏"
                        "选择最强创新焦点，讲清被淘汰的旧作战模式、装备创造的新作战模式、被逆转的常规制胜"
                        "假设、改写的交战关系以及对手新增代价；不得用‘颠覆、重构、革命性’等口号代替因果链，也不得写基线综述、技术清单、流程复述、失效、证据、"
                        "成熟度、验证或发展信息，不得使用箭头或分步骤展开。概述和制胜栏必须明确旧作战模式、新作战模式及被改写的"
                        "指挥权、兵力组织、时空存在、接敌几何或攻防交换；不得以‘颠覆、重构、革命性’等口号代替因果链。"
                        "只输出严格JSON。",
                        {
                            "parallel_card_id": f"s6-card-{position}",
                            "query": shared["topic"],

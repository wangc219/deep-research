...
def test_to_plain_handles_read_only_provider_metadata_inside_dataclass() -> None:
...
def test_baseline_packet_keeps_old_construction_and_defaults_compatibility_fields() -> None:
    ...
    assert packet.payload_type == ""
    assert packet.payload == {}


def test_v2_baseline_packet_rejects_missing_typed_payload_fields() -> None:
    packet = BaselineFindingPacket(
        packet_id="packet-v2",
        ...
        open_questions=[],
        handoff_summary="完成",
        checkpoint="weapon_equipment: complete",
        schema_version="2.0",
        payload_type="equipment_observation_v1",
        payload={"current_parameters": ["x"]},
    )

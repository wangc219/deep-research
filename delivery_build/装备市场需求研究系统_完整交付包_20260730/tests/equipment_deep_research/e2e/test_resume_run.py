...
class RejectingCompletedAgentProvider:
    def __init__(self, completed_agent_id: str) -> None:
        self.completed_agent_id = completed_agent_id
        self.delegate = FakeAgentProvider()

    def run_baseline_agent(self, request: AgentRunRequest) -> AgentRunResult:
        if request.agent.agent_id == self.completed_agent_id:
            raise AssertionError(f"completed run repeated {request.agent.agent_id}")
        return self.delegate.run_baseline_agent(request)
...
def test_resume_continues_after_last_committed_agent_savepoint(tmp_path: Path) -> None:
    ...
    with pytest.raises(InjectedCrash, match="combat_scenario|weapon_equipment"):
    ...
    checkpoint_before_resume = json.loads(
        (run_dir / "checkpoints" / "latest.json").read_text(encoding="utf-8")
    )["checkpoint"]
    completed_agents = {
        task_id.removeprefix("baseline:")
        for task_id in checkpoint_before_resume["completed_task_ids"]
        if task_id.startswith("baseline:")
    }
    pending_agents = [
        agent_id for agent_id in AGENTS if agent_id not in completed_agents
    ]
    assert database.count("EvidenceCard") == len(completed_agents)
    assert database.count("BaselineFindingPacket") == len(completed_agents)
    assert database.count("RunCheckpoint") == 1
    completed_session_prefixes = {
        agent_id: session_bytes(run_dir, agent_id)
        for agent_id in completed_agents
    }
    handoff_paths = list(run_dir.glob("handoff-failed-*.json"))
    assert len(handoff_paths) == 1
    handoff_path = handoff_paths[0]
    failure_handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    assert failure_handoff["status"] == "failed"
    assert failure_handoff["payload"]["retryable"] is True
    assert failure_handoff["payload"]["resume_checkpoint"] == "checkpoints/latest.json"
    assert failure_handoff["payload"]["transfer_candidates"]
    ...
    assert len(completing.calls) == len(pending_agents)
    assert set(completing.calls) == set(pending_agents)
    assert {
        agent_id: session_bytes(run_dir, agent_id)
        for agent_id in completed_agents
    } == completed_session_prefixes
...
def test_resume_agent_identity_is_order_insensitive() -> None:
    checkpoint = RunCheckpoint(
        run_id="resume-order",
        checkpoint_id="checkpoint-resume-order",
        completed_task_ids=[],
        pending_task_ids=[],
        round_index=1,
        budget_remaining={},
        selected_agent_ids=["combat_scenario", "weapon_equipment"],
    )

    RecoveryManager._validate_identity(
        checkpoint,
        topic=None,
        research_route=None,
        resolved_route=None,
        selected_agent_ids=["weapon_equipment", "combat_scenario"],
...
def test_completed_resume_rewrites_stable_outputs_from_sqlite(tmp_path: Path) -> None:
    ...
    resumed = build_runner(
        tmp_path,
        provider=RejectingCompletedAgentProvider(AGENTS[0]),
    ).run(
...
def test_real_session_savepoint_failure_is_reconciled_before_skipping_completed(
    ...
    pre_recovery_event_types = [row["event_type"] for row in session.read_all()]
    assert "baseline_result" in pre_recovery_event_types
    assert "savepoint" not in pre_recovery_event_types

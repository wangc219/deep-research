...
def test_fake_default_full_loop_outputs_files(tmp_path: Path) -> None:
    ...
    assert 3 <= len(summary["selected_agent_ids"]) <= 4
    ...
    for row in domain_rows:
        ...
        if row["type"] == "BaselineFindingPacket":
            assert row["payload"]["schema_version"] == "2.0"
...
def test_subset_agents_run_without_unnecessary_coverage_expansion(tmp_path: Path) -> None:
    result = _runner(tmp_path).run(
        mode="fake",
        topic="低空无人机探测预警能力缺口",
        research_route="auto",
...
def test_malformed_ipv6_source_does_not_skip_agent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    summary = json.loads(Path(result["summary_path"]).read_text(encoding="utf-8"))
    assert summary["source_materials"]
    assert all(
        item["status"] == "network_safety_rejected"
        for item in summary["source_materials"]
    )
    assert summary["store_summary"]["evidence_count"] == 0
...
agents:
  - agent_id: custom.agent_01-v2
    ...
    capability_tags: [situation, threat, scenario, equipment, operation, coordination]

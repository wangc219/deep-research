...
class ResponsesAgentProvider:
    ...
    def review_audit(self, payload: dict[str, Any]) -> dict[str, Any]:
        optimized_v2 = payload.get("execution_profile_id") == "optimized_v2"
        timeout_seconds = max(
            10.0,
            float(
                os.environ.get(
    ...
    async def _run(self, request: AgentRunRequest) -> tuple[str, dict[str, Any]]:
        ...
        optimized_v2 = (
            ...
            and request.context.get("discovery_blueprint", {}).get(
    ...
    async def _analyze_winning_subagents(
        ...
        optimized_v2 = shared["execution_profile_id"] == "optimized_v2"
        ...
        def step_shared_context(
            ...
            if optimized_v2:
                compact_blueprint["execution_profile_id"] = "optimized_v2"
        ...
        def prior_projection(
            ...
            if isinstance(dynamic_rows, list):
                projected["dynamic_inputs"] = [
                    ...
                    for item in dynamic_rows[: (2 if optimized_v2 else 3)]
                    if isinstance(item, Mapping)
        ...
        async def run_dynamic_specialist(spec: Mapping[str, Any]) -> dict[str, Any]:
            ...
            return {
                ...
                "merge_target": str(spec.get("merge_target", "S3")),
        ...
        elif dynamic_specs and not requested_resume_steps:
            accumulated["dynamic_specialists_budget_skipped"] = True
            loop_trace.append(
                {
                    "loop": "dynamic",
        ...
        # 跨步跳跃触发整段 S3-S6 回溯。
        step_dependency_map: dict[int, tuple[int, ...]] = {
            1: (),
            2: (),
            3: (1, 2),
            4: (3,),
        ...
        accumulated["dynamic_subagent_runs"] = [
            item for item in runs if item.get("execution_mode") == "dynamic"
        ]
    ...
    def _provider_for(self, agent_id: str) -> ModelProvider:
        return self.agent_providers.get(agent_id, self.provider)
...
def _report_draft_quality_issues(
    ...
    if str(payload.get("execution_profile_id", "")) == "optimized_v2":

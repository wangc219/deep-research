...
4. Agent 与 DAG 编排：只选择最小但充分的 Agent 集合；先做 capability coverage，再决定并发波次、结构化交接、关键路径和资源预算。若现有Agent不能覆盖一个可独立、可合并且预算允许的专业缺口，才生成最多3个动态专用子Agent；动态Agent只能绑定目录中存在的共享Skill、知识包和受治理Tool。不得把“常见顺序”当成硬依赖。

...
class DomainStore:
    def __init__(self) -> None:
        ...
        self.strategic_assessments: dict[str, StrategicAssessment] = {}
        self.scenario_models: dict[str, ScenarioModel] = {}
        self.equipment_observations: dict[str, EquipmentObservation] = {}
        self.operational_syntheses: dict[str, OperationalSynthesis] = {}
        self.recall_requests: dict[str, RecallRequest] = {}
        self.recommendations: dict[str, AgentRecommendation] = {}
        self.winning_inputs: dict[str, WinningMechanismInput] = {}
        self.knowledge_projections: dict[str, WinningKnowledgeProjection] = {}
        self.reasoning_nodes: dict[str, WinningReasoningNode] = {}
    ...
    def add_strategic_assessment(self, item: StrategicAssessment) -> None:
        with self._lock:
            self.strategic_assessments[item.assessment_id] = item

    def add_scenario_model(self, item: ScenarioModel) -> None:
        with self._lock:
            self.scenario_models[item.scenario_id] = item

    def add_equipment_observation(self, item: EquipmentObservation) -> None:
        with self._lock:
            self.equipment_observations[item.observation_id] = item

    def add_operational_synthesis(self, item: OperationalSynthesis) -> None:
        with self._lock:
            self.operational_syntheses[item.synthesis_id] = item
    ...
    def add_winning_input(self, item: WinningMechanismInput) -> None:
        with self._lock:
            self.winning_inputs[item.input_id] = item

    def add_knowledge_projection(self, item: WinningKnowledgeProjection) -> None:
        with self._lock:
            self.knowledge_projections[item.projection_id] = item

    def add_reasoning_node(self, item: WinningReasoningNode) -> None:
        with self._lock:
            self.reasoning_nodes[item.object_id] = item
    ...
    def evidence_snapshot(self) -> list[EvidenceCard]:
        with self._lock:
            return list(self.evidence.values())

    def baseline_packet_snapshot(self) -> list[BaselineFindingPacket]:
        with self._lock:
            return list(self.baseline_packets.values())
    ...
    def jsonl_text(self) -> str:
        ...
        rows.extend({"type": "StrategicAssessment", "payload": to_plain(item)} for item in self.strategic_assessments.values())
        rows.extend({"type": "ScenarioModel", "payload": to_plain(item)} for item in self.scenario_models.values())
        rows.extend({"type": "EquipmentObservation", "payload": to_plain(item)} for item in self.equipment_observations.values())
        rows.extend({"type": "OperationalSynthesis", "payload": to_plain(item)} for item in self.operational_syntheses.values())
        ...
        rows.extend({"type": "WinningMechanismInput", "payload": to_plain(item)} for item in self.winning_inputs.values())
        rows.extend({"type": "WinningKnowledgeProjection", "payload": to_plain(item)} for item in self.knowledge_projections.values())
        rows.extend({"type": "WinningReasoningNode", "payload": to_plain(item)} for item in self.reasoning_nodes.values())
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "strategic_assessment_count": len(self.strategic_assessments),
            "scenario_model_count": len(self.scenario_models),
            "equipment_observation_count": len(self.equipment_observations),
            "operational_synthesis_count": len(self.operational_syntheses),
            "recall_request_count": len(self.recall_requests),
            "recommendation_count": len(self.recommendations),
            "winning_input_count": len(self.winning_inputs),
            "knowledge_projection_count": len(self.knowledge_projections),
            "reasoning_node_count": len(self.reasoning_nodes),

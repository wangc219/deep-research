...
def validate_arguments(schema: dict[str, Any], arguments: dict[str, Any]) -> None:
    ...
    if schema.get("additionalProperties", True) is False:
        ...
        if extra:
            raise SchemaValidationError(f"unknown argument(s): {', '.join(extra)}")
    ...
    for key, spec in properties.items():
        ...
        if "enum" in spec:
            allowed = list(spec.get("enum") or [])
            aliases = dict(spec.get("x-enumAliases") or {})
            if value not in allowed and value not in aliases:
                allowed_text = ", ".join(str(item) for item in allowed)
                raise SchemaValidationError(
                    f"argument {key!r} must be one of: {allowed_text}"
                )

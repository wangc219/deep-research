...
Phase 1 的独立验收边界是 runtime types、无领域状态 `AgentLoop`、通用 `AgentHarness`、turn snapshot、逐调用权限、事务 save point、append-only session、预算、取消隔离和 checkpoint 恢复。这些合同均已实现并由本报告专项测试覆盖。

Phase 2-10 已在后续实现中接入 Responses provider、动态有界并发、运行时证据评分、定向再调、持久化 API/worker、Web 工作台和交付 manifest。它们复用 Phase 1 内核，但不改变本报告对 Phase 1 的完成判定。生产环境的真实公网、OIDC、PostgreSQL/Redis 高可用、TLS 和容量压测仍需在目标环境基础设施中复验。
...
本轮按 Phase 1 五项任务重新从冷启动边界执行。复验发现并修复一个受全量测试导入顺序掩盖的循环导入：`providers.base -> tools.__init__ -> tools.registry -> agents.__init__ -> agents.provider -> providers.base`。修复后 `tools` 与 `agents` 包入口不再预加载高层 registry/provider，具体对象从定义模块显式导入；新增子进程冷启动测试，保证 `AgentHarness` 在空 Python 进程中可独立导入。

Phase 1 核心矩阵：

```text
runtime types + store savepoint + AgentLoop + AgentHarness + resume + CLI workspace
213 passed in 2.14s

permission / scope / snapshot 专项
12 passed, 307 deselected in 0.24s

全项目（排除无关 legacy tests/test_responses_adapter.py）
342 passed in 2.99s
```
...
```text
python3 -m compileall -q src/equipment_deep_research tests/equipment_deep_research
git diff --check
rg -n "equipment_deep_research\.orchestration" \
  src/equipment_deep_research/harness \
  src/equipment_deep_research/tools/definitions.py
```

三项均通过；依赖方向扫描零命中。`AgentLoop` 不持有 `DomainStore`、`SqliteRunStore`、`SessionStore` 或 `ResearchRoute`。当前唯一测试告警为 FastAPI TestClient 对 `httpx` 兼容层的上游弃用提示，不影响 Phase 1 Harness 与恢复行为。

结论：Phase 1 五项计划任务完整落地，Harness 可独立冷启动、可复用、可审计、可恢复；专项测试与全项目回归通过。

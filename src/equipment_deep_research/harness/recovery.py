...
class RecoveryManager:
        ...
        *,
        ...
        try:
            ...
            self._validate_identity(
                ...
                config_fingerprint=(
                    None if allow_config_mismatch else config_fingerprint
                ),
            )
    ...
    def _load_problem(
        ...
        *,
        ...
        if (
            supplemental_information is not None
            and problem.supplemental_information != supplemental_information
        ):
        ...
        if (
            ...
            and sorted(problem.selected_agent_ids) != sorted(selected_agent_ids)
        ):
    ...
    def _validate_identity(
        ...
        *,
        ...
        if (
            selected_agent_ids is not None
            and sorted(checkpoint.selected_agent_ids) != sorted(selected_agent_ids)
        ):
    ...
    for row in rows:
        ...
        try:
            if object_type == "ResearchProblem":
                # Runs created before the benchmark subsystem was retired may
                # still contain its optional metadata. Ignore those legacy keys
                # so normal resume behavior remains backward compatible.
                payload.pop("benchmark_case_id", None)
                payload.pop("benchmark_seed", None)
            ...
            elif object_type == "StrategicAssessment":
                store.add_strategic_assessment(StrategicAssessment(**payload))
            elif object_type == "ScenarioModel":
                store.add_scenario_model(ScenarioModel(**payload))
            elif object_type == "EquipmentObservation":
                store.add_equipment_observation(EquipmentObservation(**payload))
            elif object_type == "OperationalSynthesis":
                store.add_operational_synthesis(OperationalSynthesis(**payload))
            ...
            elif object_type == "WinningMechanismInput":
                store.add_winning_input(WinningMechanismInput(**payload))
            elif object_type == "WinningKnowledgeProjection":
                store.add_knowledge_projection(
                    WinningKnowledgeProjection(**payload)
                )
            elif object_type == "WinningReasoningNode":
                store.add_reasoning_node(WinningReasoningNode(**payload))

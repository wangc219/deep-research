...
async def run_reporter_agent(
    *,
    ...
    prompt = DemandDiscoveryPromptBuilder().build(
        ContextPack(
            agent_role="reporter",
            task_brief="Write the final Chinese demand-discovery report.",
            sections={
        ...
        turn_objective=(
            "Write the Chinese demand report from the verified "
            "ReportContextBundle and call generate_demand_report exactly once."
        ),
        tools=tools,
        hard_prohibitions=[
